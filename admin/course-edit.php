...
if(!isset($_GET['course'])){
    header('Location: ./course?uid='.$uid.'&role='.$role);
    die();
}

$course = mysqli_real_escape_string($conn, $_GET['course']);
$strSQL = "SELECT * FROM rs3x_course WHERE ID = '$course' AND course_delete = 'N'";
...
<head>
  ...
  <link rel="stylesheet" href="../assets/custom/1.0.1/css/style.css" type="text/css">
</head>
        ...
        <div class="header-body">
          <div class="row align-items-center py-4">
            <div class="col-lg-6 col-7">
              <h6 class="h2 text-white d-inline-block mb-0">Update course content</h6>
            ...
            <div class="card-body pt-0">
              <form class="courseForm" onsubmit="return false;">
                <div class="form-group">
                  <label for="">Course ID : <i class="fas fa-exclamation-circle text-danger"></i></label>
                  <input type="text" name="txtCid" id="txtCid" value="<?php echo $courseData['course_id']; ?>" class="form-control text-dark" placeholder="Course ID ..." readonly>
                </div>
                <div class="form-group">
                  <label for="">Course full name : <i class="fas fa-exclamation-circle text-danger"></i></label>
                  <input type="text" name="txtTitle" id="txtTitle" value="<?php echo $courseData['course_title']; ?>" class="form-control text-dark" placeholder="Course full name / title ...">
                </div>
                <div class="form-group">
                  <label for="">Course short description : </label>
                  <textarea class="form-control" id="txtDesc" style="height: 100px;"><?php echo $courseData['course_short_desc']; ?></textarea>
                </div>
                <div class="row">
                  <div class="col-12 col-sm-6">
                    <div class="form-group">
                      <label for="">Price (USD) : <i class="fas fa-exclamation-circle text-danger"></i></label>
                      <input type="number" name="txtPrice" id="txtPrice" value="<?php echo $courseData['course_cost']; ?>" class="form-control" placeholder="Enter only number ..." min ="0" max="3000">
                    </div>
                  </div>
                  <div class="col-12 col-sm-6">
                    <div class="form-group">
                      ...
                      <select class="form-control" name="txtCat" id="txtCat">
                        ...
                        if(($result) && (mysqli_num_rows($result))){
                          while($row = mysqli_fetch_array($result)){
                            ?>
                            <option value="<?php echo $row['ID'];?>" <?php if($courseData['course_category'] == $row['ID']){ echo "selected"; } ?>><?php echo $row['cc_name']; ?></option>
                            <?php
                          }
                        }
                        ?>
                      ...
                      <div class="row">
                        <div class="col-4">
                          <select class="form-control" name="txtStartDD" id="txtStartDD">
                            ...
                            <?php
                            $start = $courseData['course_start'];
                            $start_arr = explode("-", $start);
                            for ($i=1; $i <= 31; $i++) {
                              if($i < 10){
                                ?>
                                <option value="<?php echo "0".$i; ?>" <?php if($start_arr[2] == "0".$i){ echo "selected"; } ?>><?php echo $i; ?></option>
                                <?php
                              }else{
                                ?>
                                <option value="<?php echo $i; ?>" <?php if($start_arr[2] == $i){ echo "selected"; } ?>><?php echo $i; ?></option>
                                <?php
                              }
                            }
                            ?>
                        ...
                        <div class="col-4">
                          <select class="form-control" name="txtStartMM" id="txtStartMM">
                            ...
                            for ($i=0; $i < 12; $i++) {
                              ?>
                              <option value="<?php echo $months_no[$i]; ?>" <?php if((number_format($start_arr[1]) - 1) == $i){ echo "selected"; } ?>><?php echo $months[$i]; ?></option>
                              <?php
                            }
                            ?>
                          </select>
                        ...
                        <div class="col-4">
                          <select class="form-control" name="txtStartYY" id="txtStartYY">
                            ...
                            for ($i=date('Y') + 1; $i> (date('Y') - 3); $i--) {
                              ?>
                              <option value="<?php echo $i; ?>" <?php if($start_arr[0] == $i){ echo "selected"; } ?>><?php echo $i; ?></option>
                              <?php
                            }
                            ?>
                          </select>
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control" id="txtContent" name="txtContent"><?php echo $courseData['course_desc']; ?></textarea>
                </div>
  ...
  <script type="text/javascript">
    ...
    $(function(){
      $('.courseForm').submit(function(){
        ...
        var jxr = $.post(conf.api + 'course_manage?stage=update', param, function(){}, 'json')
    ...
    <div class="modal-content">
      ...
      <div class="modal-body pt-0">
        <div class="text-center pb-3">
          ...
          <h2 class="text-center pb-0 pt-3 mb-0">Error</h2>
          <div class="p-3">
            <p>Can not update course info.</p>
            <button type="button" name="button" class="btn btn-primary btn-block" data-dismiss="modal">OK</button>
          </div>
        </div>
      </div>
